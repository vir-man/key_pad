...
void setup()
{
  ...
  // Initialize serial console for testing
  Serial.println("\n=== KEYPAD TESTING CONSOLE ACTIVE ===");
...
// Serial Console Testing System
#define MAX_CONSOLE_INPUT 100
char console_input[MAX_CONSOLE_INPUT];
uint8_t console_input_index = 0;
bool console_input_ready = false;

void serial_console_task()
{
  // Check for serial input
  while (Serial.available() > 0 && console_input_index < MAX_CONSOLE_INPUT - 1)
  {
    char c = Serial.read();
    
    if (c == '\n' || c == '\r')
    {
      console_input[console_input_index] = '\0';
    ...
    {
      console_input[console_input_index++] = c;
...
void process_console_command()
{
  ...
  while (console_input[i] != '\0' && i < MAX_CONSOLE_INPUT)
  {
    if (console_input[i] == ' ' || console_input[i] == '\t')
    {
      if (in_command)
      {
        command[j] = '\0';
    ...
    else
    {
      if (in_command)
      {
        if (j < 19) command[j++] = console_input[i];
      }
      else
      {
        if (param_count < 5 && j < 19) params[param_count][j++] = console_input[i];
      }
    }
    i++;
  }
  ...
  // Execute command
  execute_console_command(command, params, param_count);
}

void execute_console_command(char* cmd, char params[][20], uint8_t param_count)
{
  Serial.print("CMD: ");
  Serial.print(cmd);
  Serial.print(" | PARAMS: ");
  ...
  {
    test_unlock_door(params, param_count);
  }
  else if (strcmp(cmd, "lock") == 0)
  {
    test_lock_door(params, param_count);
  }
  else if (strcmp(cmd, "adduser") == 0)
  {
    test_add_user(params, param_count);
  }
  else if (strcmp(cmd, "removeuser") == 0)
  {
    test_remove_user(params, param_count);
  }
  else if (strcmp(cmd, "otp") == 0)
  {
    test_verify_otp(params, param_count);
  }
  else if (strcmp(cmd, "lostpw") == 0)
  {
    test_lost_password(params, param_count);
  }
  else if (strcmp(cmd, "changepw") == 0)
  {
    test_change_password(params, param_count);
  }
  else if (strcmp(cmd, "factory") == 0)
  {
    test_factory_reset(params, param_count);
  }
  else if (strcmp(cmd, "timeslot") == 0)
  {
    test_update_time_slot(params, param_count);
  }
...
{
  Serial.println("\n=== KEYPAD TESTING CONSOLE ===");
  Serial.println("Available commands:");
  Serial.println("  help, ?           - Show this help");
  Serial.println("  unlock <user_id> <password> - Test unlock door");
  Serial.println("  lock              - Test lock door");
  Serial.println("  adduser <user_id> <mobile> <password> - Test add user");
  Serial.println("  removeuser <user_id> - Test remove user");
  Serial.println("  otp <otp_code>    - Test verify OTP");
  Serial.println("  lostpw <user_id>  - Test lost password");
  Serial.println("  changepw <user_id> <old_pw> <new_pw> - Test change password");
  Serial.println("  factory           - Test factory reset");
  Serial.println("  timeslot <user_id> <start_hour> <end_hour> - Test time slot");
  Serial.println("  status            - Show system status");
  Serial.println("  users             - List all users");
  Serial.println("  door              - Show door status");
  Serial.println("\nExamples:");
  Serial.println("  unlock 1 1234");
  Serial.println("  adduser 2 9876543210 5678");
  Serial.println("  removeuser 2");
  Serial.println("================================\n");
}

// Test Functions for API Testing
void test_unlock_door(char params[][20], uint8_t param_count)
{
  Serial.println("Testing UNLOCK DOOR API...");
  
  if (param_count < 2)
  {
    Serial.println("ERROR: Usage: unlock <user_id> <password>");
    return;
  }
  
  // Setup parameters for API call
  uint8_t user_id = atoi(params[0]);
  char* password = params[1];
  
  // Simulate API call by setting up global variables
  _user_id_from_mobile_number = user_id - 1; // Convert to 0-based index
  para_count = 2;
  para_len[0] = 1;
  para[0][0] = user_id + '0';
  para_len[1] = strlen(password);
  strcpy(para[1], password);
  ...
  uint8_t result = api_unlock_door();
  Serial.print("Result: ");
  Serial.println(result);
}

void test_lock_door(char params[][20], uint8_t param_count)
{
  Serial.println("Testing LOCK DOOR API...");
  
  // Setup parameters for API call
  para_count = 0;
  
  // Call the actual API function
  uint8_t result = api_lock_door();
  Serial.print("Result: ");
  Serial.println(result);
}

void test_add_user(char params[][20], uint8_t param_count)
{
  Serial.println("Testing ADD USER API...");
  
  if (param_count < 3)
  {
    Serial.println("ERROR: Usage: adduser <user_id> <mobile> <password>");
    return;
  }
  
  // Setup parameters for API call
  uint8_t user_id = atoi(params[0]);
  char* mobile = params[1];
  char* password = params[2];
  
  // Simulate API call by setting up global variables
  _user_id_from_mobile_number = 0; // Master user
  para_count = 3;
  para_len[0] = 1;
  para[0][0] = user_id + '0';
  para_len[1] = strlen(mobile);
  strcpy(para[1], mobile);
  para_len[2] = strlen(password);
  strcpy(para[2], password);
  ...
  // Call the actual API function
  uint8_t result = api_add_user();
  Serial.print("Result: ");
  Serial.println(result);
}

void test_remove_user(char params[][20], uint8_t param_count)
{
  Serial.println("Testing REMOVE USER API...");
  
  if (param_count < 1)
  {
    Serial.println("ERROR: Usage: removeuser <user_id>");
    return;
  }
  ...
  // Call the actual API function
  uint8_t result = api_remove_user();
  Serial.print("Result: ");
  Serial.println(result);
}

void test_verify_otp(char params[][20], uint8_t param_count)
{
  Serial.println("Testing VERIFY OTP API...");
  
  if (param_count < 1)
  {
    Serial.println("ERROR: Usage: otp <otp_code>");
    return;
  }
  
  // Setup parameters for API call
  char* otp_code = params[0];
  
  // Simulate API call by setting up global variables
  para_count = 1;
  para_len[0] = strlen(otp_code);
  strcpy(para[0], otp_code);
  ...
  // Call the actual API function
  api_verify_otp();
  Serial.println("OTP verification completed");
}

void test_lost_password(char params[][20], uint8_t param_count)
{
  Serial.println("Testing LOST PASSWORD API...");
  
  if (param_count < 1)
  {
    Serial.println("ERROR: Usage: lostpw <user_id>");
    return;
  }
  ...
  // Call the actual API function
  api_lost_password();
  Serial.println("Lost password request completed");
}

void test_change_password(char params[][20], uint8_t param_count)
{
  Serial.println("Testing CHANGE PASSWORD API...");
  
  if (param_count < 3)
  {
    Serial.println("ERROR: Usage: changepw <user_id> <old_pw> <new_pw>");
    return;
  }
  
  // Setup parameters for API call
  uint8_t user_id = atoi(params[0]);
  char* old_password = params[1];
  char* new_password = params[2];
  ...
  para_len[0] = 1;
  para[0][0] = user_id + '0';
  para_len[1] = strlen(old_password);
  strcpy(para[1], old_password);
  para_len[2] = strlen(new_password);
  strcpy(para[2], new_password);
  
  Serial.print("User ID: ");
  Serial.println(user_id);
  Serial.print("Old Password: ");
  Serial.println(old_password);
  Serial.print("New Password: ");
  Serial.println(new_password);
  
  // Call the actual API function
  uint8_t result = api_change_password();
  Serial.print("Result: ");
  Serial.println(result);
}

void test_factory_reset(char params[][20], uint8_t param_count)
{
  Serial.println("Testing FACTORY RESET API...");
  
  // Setup parameters for API call
  _user_id_from_mobile_number = 0; // Master user
  para_count = 0;
  
  Serial.println("WARNING: This will reset all data!");
  
  // Call the actual API function
  uint8_t result = api_factory_reset();
  Serial.print("Result: ");
  Serial.println(result);
}

void test_update_time_slot(char params[][20], uint8_t param_count)
{
  Serial.println("Testing UPDATE TIME SLOT API...");
  
  if (param_count < 3)
  {
    Serial.println("ERROR: Usage: timeslot <user_id> <start_hour> <end_hour>");
    return;
  }
  
  // Setup parameters for API call
  uint8_t user_id = atoi(params[0]);
  uint8_t start_hour = atoi(params[1]);
  uint8_t end_hour = atoi(params[2]);
  ...
  para[0][0] = user_id + '0';
  para_len[1] = 2;
  sprintf(para[1], "%02d", start_hour);
  para_len[2] = 2;
  sprintf(para[2], "%02d", end_hour);
  ...
  // Call the actual API function
  api_update_time_slot();
  Serial.println("Time slot update completed");
}
...
void print_system_status()
{
  ...
  Serial.println(is_door_open() ? "OPEN" : "CLOSED");
  Serial.print("Total Users: ");
  Serial.println(MAX_NUM_OF_USERS);
  Serial.print("Current Time: ");
  // Add RTC time if available
  Serial.println("N/A");
  Serial.println("===================\n");
}
...
void print_user_list()
{
  ...
  for (uint8_t i = 0; i < MAX_NUM_OF_USERS; i++)
  {
    if (does_user_exist[i])
    {
      Serial.print("User ");
      if (i + 1 < 10) Serial.print("0");
      Serial.print(i + 1);
      Serial.print(": ");
      Serial.println(is_password_configured[i] ? "CONFIGURED" : "NOT CONFIGURED");
    }
  }
  Serial.println("================\n");
}
...
void loop()
{
  ...
  flash_drive_task();
  buzzer_task();
  serial_console_task(); // Add serial console for testing
}
